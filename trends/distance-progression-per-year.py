...
relevant_metrics = zip(
    full_metrics["date"], full_metrics["Distance"], full_metrics["Sport"]
)
...
data = [
    go.Scatter(x=list(range(366)), y=distances[year], mode="lines", name=year)
    for year in distances
]
data.append(
    go.Scatter(
        x=list(range(366)),
        y=goal_progression(GOAL_DISTANCE, 365),
        mode="lines",
        name="Goal",
    )
)
...
f = plotly.offline.plot({"data": data}, auto_open=False, filename=temp_file.name)
